...
public class SignIn implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (signed) {
			...
			UserEntity admin = new UserEntity("admin", "admin@admin.com", "avatar", User.MANAGER_ROLE,
					Playground.PLAYGROUND_NAME);
			model.setCurrentUser(admin);
			frame.dispose();
			new MainFrame(model);
		} else if (userPlayground_text.equals("player") && email_text.equals("test")) {
			UserEntity admin = new UserEntity("player", "player@admin.com", "avatar", User.PLAYER_ROLE,
					Playground.PLAYGROUND_NAME);
			model.setCurrentUser(admin);
			frame.dispose();
			new MainFrame(model);
		} else
			new errorMessage(Client.SIGN_IN_INVALID);
