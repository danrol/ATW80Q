...
//******************************************************************************************//
	...
	// 1.1 Scenario: Test register with wrong email (no �@� and web address
	// afterwards)
	@Test(expected = RuntimeException.class)
	public void registerNewUserWithWrongEmail() {

		NewUserForm postUserForm = new NewUserForm("WrongEmail", User.DEFAULT_USERNAME, User.AVATAR_FOR_TESTS,
				User.PLAYER_ROLE);
	...
	public void successfullyRegisterNewUser() {
		...
		NewUserForm postUserForm = new NewUserForm(User.EMAIL_FOR_TESTS, User.DEFAULT_USERNAME, User.AVATAR_FOR_TESTS,
				User.PLAYER_ROLE);
		UserTO testValue = new UserTO(new UserEntity(postUserForm.getUsername(), postUserForm.getEmail(),
				postUserForm.getAvatar(), postUserForm.getRole(), Playground.PLAYGROUND_NAME));
		UserTO actualReturnedValue = this.restTemplate.postForObject(this.url + Playground.Function_1, postUserForm,
				UserTO.class);
		assertThat(actualReturnedValue).isNotNull().isEqualToComparingFieldByField(testValue);
	}

	// 1.3 Scenario: Test register user with the same email as the one that another
	// user already have in the database
	@Test(expected = RuntimeException.class)
	...
	public void confirmUserEmailNotInDatabase() {
		...
		this.restTemplate.getForObject(this.url + Playground.Function_2, UserTO.class, Playground.PLAYGROUND_NAME,
				User.EMAIL_FOR_TESTS, User.DEFAULT_VERIFICATION_CODE);
	}
	...
	public void confirmUserWithCorrectCode() {
		...
		UserTO user = this.restTemplate.getForObject(this.url + Playground.Function_2, UserTO.class,
				Playground.PLAYGROUND_NAME, User.EMAIL_FOR_TESTS, u.getVerificationCode());
	...
	public void confirmUserNotInPlayground() {
		...
		UserEntity u = new UserEntity(User.DEFAULT_USERNAME, User.EMAIL_FOR_TESTS, User.AVATAR_FOR_TESTS,
				User.MANAGER_ROLE, Playground.PLAYGROUND_NAME);
		this.userService.addUser(u);
		this.restTemplate.getForObject(this.url + Playground.Function_2, UserTO.class, Playground.Other_Playground,
				User.EMAIL_FOR_TESTS, u.getVerificationCode());
	}
	...
	public void confirmUserWithIncorrectVerificationCode() {
		...
		this.userService.addUser(u);
		this.restTemplate.getForObject(this.url + Playground.Function_2, UserTO.class, Playground.PLAYGROUND_NAME,
				User.EMAIL_FOR_TESTS, u.getVerificationCode() + "x");
	}
	...
	public void loginUserWithCorrectEmail() {
		...
		u.verifyUser();
		this.userService.addUser(u);
		UserTO user = this.restTemplate.getForObject(this.url + Playground.Function_3, UserTO.class,
	...
	public void loginUserEmailNotInDatabase() {
		...
		this.restTemplate.getForObject(this.url + Playground.Function_3, UserTO.class, Playground.PLAYGROUND_NAME,
				User.EMAIL_FOR_TESTS);
	}
	...
	public void loginUserNotInPlayground() {
		...
		UserEntity u = new UserEntity(User.DEFAULT_USERNAME, User.EMAIL_FOR_TESTS, User.AVATAR_FOR_TESTS,
				User.MANAGER_ROLE, Playground.Other_Playground);
		u.verifyUser();
		this.userService.addUser(u);
		this.restTemplate.getForObject(this.url + Playground.Function_3, UserTO.class, Playground.PLAYGROUND_NAME,
				User.EMAIL_FOR_TESTS);
	...
	public void UserUpdateHisInfo() {
		...
		UserEntity managerUser = new UserEntity(User.DEFAULT_USERNAME, User.EMAIL_FOR_TESTS, User.AVATAR_FOR_TESTS,
				User.MANAGER_ROLE, Playground.PLAYGROUND_NAME);
		managerUser.verifyUser();
		this.userService.addUser(managerUser);
		managerUser.setAvatar("name");
		this.restTemplate.put(this.url + Playground.Function_4, new UserTO(managerUser), managerUser.getPlayground(),
	...
	public void UserUpdateAnotherUser() {
		...
		UserEntity otherUser = new UserEntity(User.DEFAULT_USERNAME, "other" + User.EMAIL_FOR_TESTS,
		...
		this.restTemplate.put(this.url + Playground.Function_4, new UserTO(otherUser), managerUser.getPlayground(),
				managerUser.getEmail());
	}
